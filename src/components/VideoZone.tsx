...
const VideoZone = ({ currentMinute, videoUrl = '', onVideoUrlChange, onCurrentTimeChange }: VideoZoneProps) => {
  ...
  // Track current time and notify parent
  useEffect(() => {
    if (onCurrentTimeChange) {
      // For MVP: convert currentMinute to seconds
      // In production, use YouTube IFrame API for accurate time tracking
      const timeInSeconds = currentMinute * 60;
      onCurrentTimeChange(timeInSeconds);
    }
  }, [currentMinute, onCurrentTimeChange]);
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        <Input
          ...
          className="bg-black/40 border-chalk-white/20 text-chalk-white placeholder:text-chalk-white/40 focus-visible:ring-chalk-green focus-visible:border-chalk-green"
        />
        {error && (
        ...
        {videoSource && !error && (
          <p className="text-chalk-white/60 text-xs font-body px-1">
            {getVideoSourceLabel()} video loaded
          </p>
        )}
      </div>

      {/* Video container */}
      <div className="relative rounded-lg overflow-hidden chalk-border bg-black/40">
        <div className="aspect-video bg-gradient-to-br from-chalk-green-light/30 to-black/50 relative">
          {videoSource && videoSource.type === 'youtube' ? (
            /* YouTube embed */
            <iframe
              ref={iframeRef}
              src={`https://www.youtube.com/embed/${videoSource.videoId}?autoplay=0&controls=1&enablejsapi=1`}
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-full h-full bg-gradient-to-br from-emerald-900/50 to-black/70 flex items-center justify-center">
                  ...
                  <div className="relative w-3/4 h-3/4 border border-chalk-white/20 rounded">
                    ...
                    <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-chalk-white/30 font-chalk text-2xl">
                      {videoSource ? 'Video Source Not Supported' : 'Enter a video URL above'}
                    </span>
                  </div>
                </div>
              </div>
      ...
      {/* Watching label */}
      {videoSource && (
        <p className="text-chalk-white/60 text-xs font-body">
          Watching via {getVideoSourceLabel()}
        </p>
      )}
    </div>
  );
